...
class ToolCallingAgent:
    ...
    def _init_ollama(self):
        ...
        try:
            import ollama
            from .agents.ollama_agent import OllamaNativeAgent
            ...
            try:
                ...
                if model not in available_models:
                    logger.warning(
                        f"Recommended model {model} not found in available models"
                    )
    ...
    def chat(
        self, message: str, use_tools: bool = True, stream: bool = False, **kwargs
    ) -> str:
        """
        ...
        if stream:
            ...
            for chunk in agent.chat(task, stream=True):
                ...
                if chunk_type == "thinking":
                    ...
                    tool_info = content
                    print(
                        f"  → {tool_info.get('name', 'unknown')}: {tool_info.get('arguments', {})}"
                    )
            ...
            if user_input.lower() == "/exit" or user_input.lower() == "quit":
                ...
                try:
                    ...
                    if 1 <= sample_num <= len(sample_tasks):
                        ...
                        print(
                            f"❌ Invalid sample number. Please choose between 1 and {len(sample_tasks)}"
                        )
...
def main():
    ...
    if args.mode == "single":
        if not args.task:
            ...
            try:
                choice = input(
                    f"\nSelect a task number (1-{len(sample_tasks)}) or 'q' to quit: "
                ).strip()
                if choice.lower() == "q":
                    return 0
                ...
                if 1 <= task_num <= len(sample_tasks):
                    ...
                    if confirm == "y":
                        stream_enabled = (
                            not args.no_stream if hasattr(args, "no_stream") else True
                        )
                        run_single_task(
                            agent, selected_task["task"], stream=stream_enabled
                        )
                    else:
